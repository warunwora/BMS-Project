...
export default function ProductEdit() {
  ...
  useEffect(() => { get(`/products/${id}`).then(setForm).catch((e) => toast(e.message, "error")); }, [id]);

  function set(k, v) { setForm((f) => ({ ...f, [k]: v })); }

  async function doSave() {
    try { await put(`/products/${id}`, form); toast("Product updated successfully"); nav("/data/products"); }
