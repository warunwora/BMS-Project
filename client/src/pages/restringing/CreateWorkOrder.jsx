...
export default function CreateWorkOrder() {
  ...
  const [newItem, setNewItem] = useState({ asset: "", selectedProduct: null, service: "Stringing", tension: "", material_cost: "0.00", labor_fee: "0.00" });
  const [errors, setErrors] = useState({});

  function setItem(k, v) { setNewItem((i) => ({ ...i, [k]: v })); }

  const BRANDS = ["Yonex", "Viktor", "Karakal", "Li-Ning", "Wilson", "Babolat", "Ashaway", "Sotx", "Carlton"];

  function extractBrand(name = "") {
    const first = name.split(" ")[0];
    return BRANDS.includes(first) ? first : "-";
  }
  ...
  function handleProductSelect(p) {
    setNewItem((i) => ({
      ...i,
      selectedProduct: p,
      asset: p ? extractBrand(p.name) : i.asset,
      material_cost: p ? String(p.unit_price ?? "0.00") : "0.00",
    }));
  }
  ...
  function addItem() {
    const row = {
      asset: newItem.asset,
      product_code: newItem.selectedProduct?.code ?? "",
      service: newItem.service,
      tension: newItem.tension,
      material_cost: newItem.material_cost,
      labor_fee: newItem.labor_fee,
    };
    setItems((prev) => [...prev, row]);
    setNewItem({ asset: "", selectedProduct: null, service: "Stringing", tension: "", material_cost: "0.00", labor_fee: "0.00" });
  }
  ...
  async function handleSave() {
    ...
    if (!technician) e.techId = "Technician is required";
        ...
        <div className="col-span-2 flex flex-col gap-6">
          <Card title="Order Details">
            ...
            <div className="grid grid-cols-2 gap-6 mt-5">
              <Field label="Technician" required>
                <TechnicianSearch selected={technician} onSelect={(t) => { setTechnician(t); setErrors((p) => ({ ...p, techId: "" })); }} />
          ...
          <Card title="Service Items" action={<Button variant="outlineBlue" icon={Plus} onClick={addItem}>Add Service</Button>}>
            <div className="grid grid-cols-3 gap-3 mb-4">
              <Field label="Asset (Racket Model)">
                <Input placeholder="e.g. Yonex Astrox 99" value={newItem.asset} onChange={(e) => setItem("asset", e.target.value)} />
              </Field>
              <Field label="Product (String/Grip)">
                <ProductSearch selected={newItem.selectedProduct} onSelect={handleProductSelect} categories="string,grip,accessory" />
              </Field>
              <Field label="Service">
              ...
              <Field label="Tension (lbs)"><Input placeholder="0" value={newItem.tension} onChange={(e) => setItem("tension", e.target.value)} /></Field>
              <Field label="Mat. Cost">
                <Input placeholder="0.00" value={newItem.material_cost} onChange={(e) => setItem("material_cost", e.target.value)} />
              </Field>
