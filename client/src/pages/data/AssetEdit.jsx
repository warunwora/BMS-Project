...
export default function AssetEdit() {
  ...
  useEffect(() => { get(`/assets/${id}`).then((data) => { setForm(data); setOriginal(data); }).catch((e) => toast(e.message, "error")); }, [id]);

  function set(k, v) { setForm((f) => ({ ...f, [k]: v })); }

  async function doSave() {
    try { await put(`/assets/${id}`, form); toast("Asset updated successfully"); setOriginal(form); nav("/data/assets"); }
