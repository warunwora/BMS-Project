...
export default function CourtEdit() {
  ...
  useEffect(() => { get(`/courts/${id}`).then((data) => { setForm(data); setOriginal(data); }).catch((e) => toast(e.message, "error")); }, [id]);

  function set(k, v) { setForm((f) => ({ ...f, [k]: v })); }

  async function doSave() {
    try { await put(`/courts/${id}`, form); toast("Court updated successfully"); setOriginal(form); nav("/data/courts"); }
