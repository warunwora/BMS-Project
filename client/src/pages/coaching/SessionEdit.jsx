...
export default function SessionEdit() {
  ...
  useEffect(() => { get(`/sessions/${id}`).then((data) => { setForm(data); setOriginal(data); }).catch((e) => toast(e.message, "error")); }, [id]);

  function set(k, v) { setForm((f) => ({ ...f, [k]: v })); }

  async function doSave() {
    try {
      const { member, coach, coaching_slot, ...fields } = form;
      await put(`/sessions/${id}`, fields);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <PageTitle back title={`Edit Session ${form.code}`} />
      ...
      <div className="flex flex-col gap-6">
        ...
        <Card title="Session Info">
          <div className="grid grid-cols-3 gap-6 mb-5">
            <Info label="Coach"      value={form.coach?.name} />
            <Info label="Speciality" value={form.coach?.speciality} />
            <Field label="Skill Focus">
              <Input value={form.skill_focus ?? ""} onChange={(e) => set("skill_focus", e.target.value)} />
            </Field>
          </div>
          <Field label="Booking Date">
            <input type="date" value={form.booking_date ?? ""} onChange={(e) => set("booking_date", e.target.value)} className="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200" />
          </Field>
        </Card>
        <Card title="Payment">
          <div className="grid grid-cols-3 gap-6">
      ...
      <ConfirmModal open={confirm} title="Save Changes?" message="Save updates to this session?" confirmText="Save" variant="primary" onConfirm={doSave} onCancel={() => setConfirm(false)} />
