...
export function FilterPills({ items, value, onChange, icons = {} }) {
  return (
      ...
      {items.map((it) => {
        ...
        return (
          <button
            key={it}
            onClick={() => onChange?.(it)}
            className={`inline-flex items-center gap-2 px-5 py-2 rounded-full text-sm font-medium border transition-all duration-150 active:scale-95 ${
              value === it
...
export function Pagination({ page = 1, total = 1, onPage, totalRows = 0, rowsPerPage = 20, onRowsPerPage }) {
  const totalPages = Math.max(1, Math.ceil(totalRows / rowsPerPage));
  const currentPage = Math.min(page, totalPages);

  return (
    <div className="sticky bottom-0 z-10 bg-white border-t border-slate-100 py-3 mt-6 flex items-center justify-between">
      <div className="flex items-center gap-3">
        <select
          value={rowsPerPage}
          onChange={(e) => { onRowsPerPage?.(Number(e.target.value)); onPage?.(1); }}
          className="px-3 py-1.5 rounded-lg border border-slate-200 text-sm text-slate-600 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-200 cursor-pointer"
        >
          {PAGE_SIZES.map((s) => <option key={s} value={s}>{s} rows</option>)}
        </select>
        <span className="text-sm text-slate-500">
          Page {currentPage} of {totalPages}
          {totalRows > 0 && <span className="ml-2 text-slate-400">({totalRows.toLocaleString()} total)</span>}
        </span>
      </div>
      <div className="flex gap-2">
        {[
          [ChevronsLeft,  () => onPage?.(1)],
          [ChevronLeft,   () => onPage?.(Math.max(1, currentPage - 1))],
          [ChevronRight,  () => onPage?.(Math.min(totalPages, currentPage + 1))],
          [ChevronsRight, () => onPage?.(totalPages)],
        ].map(([Ic, handler], i) => (
          <button key={i} onClick={handler} className="w-10 h-10 rounded-lg border border-slate-200 flex items-center justify-center hover:bg-slate-50 transition-all active:scale-90 disabled:opacity-40" disabled={!onPage}>
      ...
      <div className="grid items-center text-sm py-4 px-2" style={{ gridTemplateColumns: gridTpl }}>
        {columns.map((c) => (
          <button
            ...
            className={`text-left w-full flex items-center gap-1 select-none transition-colors hover:text-slate-700 group ${sortKey === c.key ? "text-indigo-500" : "text-slate-400"}`}
          >
            {c.label}
            {sortKey === c.key
...
export function Tooltip({ children, text, position = "top" }) {
  ...
  return (
    <div className="relative group inline-flex">
      {children}
      <div className={`no-print pointer-events-none absolute ${posClass[position]} px-2.5 py-1 text-xs text-white bg-slate-800 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-all duration-150 z-50 shadow-lg`}>
        {text}
  ...
  return (
    <div ref={btnRef}>
      <Tooltip text={`Filter by ${label}`}>
        <button
          onClick={toggle}
          className={`inline-flex items-center justify-between gap-3 px-4 py-3 border rounded-xl text-sm bg-white min-w-[140px] max-w-[200px] transition-all active:scale-95 ${active ? "border-indigo-400 text-indigo-600" : "border-slate-200 text-slate-700 hover:border-slate-300"}`}
        >
          <span className="truncate max-w-[140px]">{active ? value : label}</span>
      ...
      {open && rect && createPortal(
        <div
          ...
          className="bg-white border border-slate-200 rounded-xl shadow-lg py-1 min-w-[160px] max-w-xs max-h-64 overflow-y-auto animate-fade-in"
        >
          {["All", ...options].map((opt) => (
            <button
              key={opt}
              onMouseDown={(e) => { e.preventDefault(); onChange(opt === "All" ? "" : opt); setOpen(false); }}
              className={`w-full text-left px-4 py-2.5 text-sm transition hover:bg-slate-50 truncate ${(active ? value === opt : opt === "All") ? "text-indigo-600 font-medium bg-indigo-50" : "text-slate-700"}`}
            >
              {opt}
            </button>
          ))}
...
export function MemberSearch({ onSelect, selected, placeholder = "Search name, ID or phone" }) {
  ...
  useEffect(() => {
    function outside(e) {
      const inC = containerRef.current && containerRef.current.contains(e.target);
      const inP = portalRef.current && portalRef.current.contains(e.target);
      if (!inC && !inP) setOpen(false);
    }
    document.addEventListener("mousedown", outside);
    return () => document.removeEventListener("mousedown", outside);
  }, []);
  ...
  async function fetchMembers(q = "") {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE}/api/members?search=${encodeURIComponent(q)}`);
  ...
  useEffect(() => {
    const q = query.trim();
    if (!q) return;
    const timer = setTimeout(() => fetchMembers(q), 250);
      ...
      {selected ? (
        ...
        <div className="relative">
          ...
          <input
            ...
            onFocus={() => fetchMembers(query.trim())}
          />
      ...
      {open && rect && createPortal(
        ...
        >
          {results.length === 0 ? (
            ...
            <>
              <div className="grid text-xs text-slate-400 px-4 py-2 border-b border-slate-100" style={{ gridTemplateColumns: "0.5fr 2fr 1.5fr 1fr 0.8fr" }}>
                <div>ID</div><div>Name</div><div>Phone</div><div>Tier</div><div>Points</div>
              </div>
              <div className="max-h-52 overflow-y-auto">
                {results.map((m) => (
                  <button
                    ...
                    style={{ gridTemplateColumns: "0.5fr 2fr 1.5fr 1fr 0.8fr" }}
                  >
                    <div className="text-slate-400">{m.id}</div>
...
export function CoachSearch({ onSelect, selected }) {
  ...
  useEffect(() => {
    const q = query.trim();
    if (!q) return;
    const timer = setTimeout(() => fetchCoaches(q), 250);
    return () => clearTimeout(timer);
  }, [query]);

  async function fetchCoaches(q = "") {
    try {
      const res = await fetch(`${API_BASE}/api/coaches?search=${encodeURIComponent(q)}`);
      const json = await res.json();
      const data = json?.data ?? json;
      setResults(Array.isArray(data) ? data : []);
      setOpen(true);
      ...
      {selected ? (
        ...
        <div className="relative">
          ...
          <input
            ...
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            onFocus={() => fetchCoaches(query.trim())}
          />
        </div>
      )}
      {open && rect && createPortal(
...
export function ProductSearch({ onSelect, selected, categories = "" }) {
  const [query, setQuery] = useState("");
  const [results, setResults] = useState([]);
  const [open, setOpen] = useState(false);
  const [rect, setRect] = useState(null);
  ...
  async function fetchProducts(q = "") {
    try {
      const qs = new URLSearchParams({ search: q, ...(categories ? { categories } : {}) });
  ...
  useEffect(() => {
    const q = query.trim();
    if (!q) return;
    const timer = setTimeout(() => fetchProducts(q), 250);
    return () => clearTimeout(timer);
  }, [query]);
  ...
  return (
    <div ref={containerRef}>
      {selected ? (
        <div className="flex items-center gap-3 px-4 py-2.5 border border-indigo-300 rounded-xl bg-indigo-50">
          <div className="flex-1 min-w-0">
            <span className="font-semibold text-indigo-700 text-sm">{selected.code}</span>
            <span className="text-indigo-400 text-sm ml-2">{selected.name}</span>
            {selected.unit_price && <span className="ml-2 text-xs text-indigo-400">฿{selected.unit_price}</span>}
          </div>
          <button onClick={handleClear} className="text-indigo-300 hover:text-rose-500 transition-colors"><X className="w-4 h-4" /></button>
        </div>
      ) : (
        <div className="relative">
          ...
          <input
            ...
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            onFocus={() => fetchProducts(query.trim())}
          />
        </div>
      )}
      {open && rect && createPortal(
        ...
        >
          {results.length === 0 ? (
            ...
            <>
              <div className="grid text-xs text-slate-400 px-4 py-2 border-b border-slate-100" style={{ gridTemplateColumns: "1fr 2fr 1fr" }}>
                <div>Code</div><div>Name</div><div>Price</div>
              </div>
              <div className="max-h-52 overflow-y-auto">
                {results.map((p) => (
                  <button
                  ...
                  >
                    <div className="font-semibold text-indigo-600">{p.code}</div>
                    <div className="text-slate-900">{p.name}</div>
                    <div className="text-slate-500">฿{p.unit_price}</div>
                  </button>
                ))}
              </div>
            </>
...
export function TechnicianSearch({ onSelect, selected }) {
  const [query, setQuery] = useState("");
  const [results, setResults] = useState([]);
  const [open, setOpen] = useState(false);
  const [rect, setRect] = useState(null);
  ...
  async function fetchTechnicians(q = "") {
    try {
      const res = await fetch(`${API_BASE}/api/technicians?search=${encodeURIComponent(q)}`);
      const json = await res.json();
      const data = json?.data ?? json;
      setResults(Array.isArray(data) ? data : []);
      setOpen(true);
  ...
  useEffect(() => {
    const q = query.trim();
    if (!q) return;
    const timer = setTimeout(() => fetchTechnicians(q), 250);
    return () => clearTimeout(timer);
  }, [query]);
    ...
    <div ref={containerRef}>
      {selected ? (
        <div className="flex items-center gap-3 px-4 py-2.5 border border-indigo-300 rounded-xl bg-indigo-50">
          <div className="flex-1 min-w-0">
            <span className="font-semibold text-indigo-700 text-sm">{selected.name}</span>
            {selected.code && <span className="text-indigo-400 text-sm ml-2">{selected.code}</span>}
          </div>
          <button onClick={handleClear} className="text-indigo-300 hover:text-rose-500 transition-colors"><X className="w-4 h-4" /></button>
        </div>
      ) : (
        <div className="relative">
          ...
          <input
            ...
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            onFocus={() => fetchTechnicians(query.trim())}
          />
        </div>
      )}
      {open && rect && createPortal(
        ...
        >
          {results.length === 0 ? (
            ...
            <>
              <div className="grid text-xs text-slate-400 px-4 py-2 border-b border-slate-100" style={{ gridTemplateColumns: "0.5fr 2fr 1fr" }}>
                <div>ID</div><div>Name</div><div>Code</div>
              </div>
              <div className="max-h-52 overflow-y-auto">
                {results.map((t) => (
                  <button
                    ...
                    className="w-full text-left grid px-4 py-3 hover:bg-indigo-50 text-sm border-b border-slate-100 last:border-0 transition-colors"
                    style={{ gridTemplateColumns: "0.5fr 2fr 1fr" }}
                  >
                    <div className="text-slate-400">{t.id}</div>
